    ...
    void Start()
    {
        ...
        if (!mainCamera.GetComponent<MainCameraLayerManager>())
        {
            ...
            foreach (Camera cam in cameras)
            {
                ...
                {
                    Debug.Log("here");
                    cam.gameObject.AddComponent<MainCameraLayerManager>();  // this allows us to alter layers before / after a render!
                }
            }
        }
	...
	public void SwitchDimensions ()
	{
       ...
       // DimensionChanger.SwitchDimensions(this.mainCamera.gameObject, FromDimension(), ToDimension());
        ...
        foreach (Camera cam in cameras)
        {
            if(cam.tag == "MainCamera")
            {
                DimensionChanger.SwitchCameraRender(cam, FromDimension().layer, ToDimension().layer, ToDimension().customSkybox);
                DimensionChanger.SwitchDimensions(cam.gameObject, FromDimension(), ToDimension());
            }           
